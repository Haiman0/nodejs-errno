...
/**
 * @class
 * @classdesc A container of errors.
 * @param {object} options - options
 * @param {string} options.lockFile - an absolute path 
 */
 ...
 * @param  {(strin|Domain)}	- domain
 * @param  {string}
 * @param  {function}
 * @return {object} - defined information
 */
...
fn._generateLockFileData = function (){
	var data;
...
/**
 * Lock the container state, so as not to change the err code in different circumstances
...
fn.lock = function (cb){
	...
	var data = this._generateLockFileData()
...
fn.lockSync = function (){
	...
	fs.writeFileSync(this._lockFile, data)
}

/**
 * Generate a specified error 
 * @param  {string|Domain}
 * @param  {...*} data - pass to factory datas
 * @return {*}
 */
...
/**
 * Identify whether an error instance belongs to a defined error type
 * @param  {object} - a error instace
 * @param  {string|Domain} - 
 * @return {boolean}
 */
